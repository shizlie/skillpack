...
9. [ ] Deploy lane next: add CI/CD for hosted `apps/api` and `apps/dashboard` so frontend and backend can deploy independently but stay env-wired together (`SKILLPACK_API_BASE_URL`, dashboard origin, Clerk config, management key injection).
